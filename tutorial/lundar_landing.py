...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        state = [(pos.x - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2),
                 ...
                 vel.y * (VIEWPORT_H / SCALE / 2) / FPS, self.lander.angle,
...
def heuristic(env, s):
    ...
    hover_targ = 0.55 * np.abs(
        s[0])  # target y should be proportional to horizontal offset
    ...
    if s[6] or s[7]:  # legs have contact
        ...
        hover_todo = -(
            s[3]
        ) * 0.5  # override to reduce fall speed, that's all we need after contact
